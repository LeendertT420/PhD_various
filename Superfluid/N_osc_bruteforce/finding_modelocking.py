...
params_unitless = to_unitless(config2)



delta = -3
print(f'bifurcation: {upper_boundary(N, delta)}')

params = {'N': N,
            'gamma': params_unitless['gamma'],
            'mu': mu_spectrum(N),
            'tau': params_unitless['tau'],
            'alpha': 0.34,
            'delta': delta,
            'sigma': params_unitless['sigma'],
...
Dt_sim = 10000.0
t_span = (0.0, Dt_sim)
N_points = int(20*(t_span[1]-t_span[0])*params['mu'][-1]/(2*np.pi))
...
plt.plot(t_eval, np.sum(sol.y[:N,:], axis=0), c='k')
for i in range(N):
    plt.plot(t_eval, sol.y[i,:], alpha=.4)
    
plt.xlabel('time')
plt.ylabel('superfluid film displacement')
plt.show()
...
if Lyapunov:
    ...
    saturation_idx = np.where(distance >= 1)[0][0]
